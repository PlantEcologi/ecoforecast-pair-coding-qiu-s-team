...
> *Answer 1: Although fynbos greenery increases over time linearly, NDVI is measured using satellite data. This means that is will capture the extent of greenery over an area but no the underlying density of greenery. Once an area is covered with enough vegetation, more vegetation grown in underlying layers that increases the density of the canopy will not be reflected in the NDVI data. This is seen by the logarithmic trend of the graph. 
Another factor that could play a role is the exponential increase in plant growth after a fire. Fires return nutrients to the soil, allowing fynbos to grow rapidly since fynbos is a nutrient-limited family. After a while, the nutrients in the soil have been absorbed by the plants and there is no longer enough to allow for the same growth rate as before. This means the growth rate will slow, which slows the rate at which greenery occurs, therefore lowering the gradient of the NDVI curve, producing this logistic trendline.*
...
> *Alpha: The estimate of α of the full model is 0.2075 which is less than the simple model estimate, 0,2511. This indicates that the full model assumes the initial NDVI is lower compared to the simple model as α is the NDVI at time 0. The simple models' α value is given by  
Gamma:The estimate γ of the full model is 0.3647 which is higher than the simple model with 0,3237. This means the full model predicts a higher maximum average increase in NDVI. Since both the simple and full model must take into account high NDVI readings in the later years, the lower α value in the full model causes its' γ value to be higher to reach the same NDVI values within the same time interval as the simple model.
LAmbda: λ is the rate of increase in NDVI. The simple model has a higher λ value of 1.17687 compared to the full model. The full model has a lower rate of increase at 0.9892 since it takes into account decreasing NDVI due to periodic seasonality. The simple model does not account for seasonality, therefore it is not influenced by the decreasing NDVI in winter and has a higher λ value.*
...
> *The large F value indicates there is a significant difference beween the two models. The low p-value of 4.12X10^14 indicates that there are multiple significant differences between the models. The full model has a residual sum of squares of 0.4528 which is lower than the simple models residual. This means that the Full model is comparatively better at predicting the data. This, coupled with the signficance of the p-value, indicates that the full model is significantly better at predicting the NDVI data compared to the Simple model. 
*

Q4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)?

> *There are two degrees of freedom, A whcih is the amplitude of the sine term, and ϕ which adjusts the timing of the sine term to account for the month the fire occurred.*
...
> *We prefer the Full model since it has a lower Akaike’s information criterion. *
...
> *Akaike's information criterion is computed using Maximum likelihood estimation (MLE). Since the Nonlinear Least Squares method fails to maximise likelihoods, the AIC values, although can be computed, should not be compared since the assumptions of the formula will be false.  *
